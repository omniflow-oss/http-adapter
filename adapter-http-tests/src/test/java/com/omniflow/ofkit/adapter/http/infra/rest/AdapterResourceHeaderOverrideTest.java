...
class AdapterResourceHeaderOverrideTest {
    @Test
    void header_base_overrides_profile_base() throws Exception {
        ...
        UriInfo uriInfo = uri("/get?a=1");
        HttpHeaders headers = new HeadersWith(Map.of("X-OF-Target-Base", List.of("https://httpbin.org")));

        Response r = res.get("p", "get", uriInfo, headers);
        assertEquals(200, r.getStatus());
        assertNotNull(seen[0]);
        assertEquals("https://httpbin.org/get?a=1", seen[0].uri().toString());
    }

    private static UriInfo uri(String pathAndQuery) {
        return new UriInfo() {
            @Override public String getPath() { return pathAndQuery; }
            ...
            @Override public URI getRequestUri() { return URI.create("http://x" + pathAndQuery); }
            @Override public UriBuilder getRequestUriBuilder() { return UriBuilder.fromUri(getRequestUri()); }
            @Override public URI getAbsolutePath() { return URI.create("http://x" + pathAndQuery.split("\\?")[0]); }
    ...
    static class HeadersWith implements HttpHeaders {
        final MultivaluedMap<String, String> m = new MultivaluedHashMap<>();
        HeadersWith(Map<String, List<String>> init) { init.forEach((k,v) -> m.put(k, new ArrayList<>(v))); }
        @Override public List<String> getRequestHeader(String name) { return m.get(name); }
        @Override public String getHeaderString(String name) { var v = m.get(name); return (v==null||v.isEmpty())? null : v.get(0); }
